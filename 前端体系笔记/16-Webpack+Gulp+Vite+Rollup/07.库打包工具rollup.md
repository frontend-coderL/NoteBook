...
- 另外webpack通常可以通过各种 loader处理各种各样的文件 ,以及处理它们的依赖关系;
- rollup更多时候是专注于处理JavaScript代码的(当然也可以处理css 、font 、vue等文件); 
- 另外 rollup的配置和理念相对于webpack来说,更加的简洁和容易理解;
- 在早期webpack不支持tree shaking时, rollup具备更强的优势;

目前webpack和 rollup分别应用在什幺场景呢?

- 通常在实际项目开发过程中,我们都会使用webpack (比如 react 、angular项目都是基于webpack的);
- 在对库文件进行打包时,我们通常会使用 rollup (比如vue 、 react 、 dayjs源码本身都是基于 rollup的, Vite底层使用Rollup);
...
我们可以将配置信息写到配置文件中 rollup.config.js文件: `npx rollup -c`
...
如果我们希望将ES6转成ES5的代码,可以在 rollup中使用babel。

- 安装rollup对应的babel 插件:  `npm install @rollup/plugin-babel -D`
- 修改配置文件:
...
处理vue文件我们需要使用 rollup-plugin-vue插件:

- 但是注意:默认情况下我们安装的是vue2.x的版本,所以我这里指定了一下 rollup-plugin-vue的版本;
...
![image-20230823220638052](./assets/image-20230823220638052.png)

这是因为在我们打包的vue代码中,用到 process.env.NODE_ENV ,所以我们可以使用一个插件 rollup-plugin-replace 设置它对应的值:  `npm install @rollup/plugin-replace -D`
...
## 五、rollup本地服务器
 ...
 第一步:使用 rollup-plugin-serve搭建服务
