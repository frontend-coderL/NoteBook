...
- [mattpocock/skills](https://github.com/mattpocock/skills) ⭐：一套前端开发工具，解决人和 Agent 之间沟通对不齐的问题
  - **开发类**
    ...
    - `/grill-with-docs` ⭐：逐个分支拷问你的计划，对标项目已有术语表（CONTEXT.md）和架构决策记录（ADRs），同时实时更新。
      - 使用时注意：低保真问题直接 grill，高保真问题交接给 `/prototype`；任务范围宜小不宜大；主动引导而非被动回答；用 `/to-prd` 保存设计决策而非清空重来；并行执行多个对话提效。
...
- [garden-skills](https://github.com/ConardLi/garden-skills) ⭐：一套偏设计与内容生产方向的 Agent Skills 集合，适合提升网页生成审美、演示表达和知识检索效率。
  - `/web-design-engineer`：网页设计工程师，按设计流程生成更有风格区分度的产品官网、活动页和展示页。
  - `/web-video-presentation`：把文章、稿子或提纲转成 16:9 网页演示，适合录视频、做分享和产品展示。
  - `/gpt-image-2`：结构化图像提示词生成，适合海报、UI 样机、技术插图等视觉素材生成场景。
  - `/kb-retriever`：本地知识库检索，按需定位相关资料，减少把大量文档直接塞进上下文。
...
- <https://github.com/Jesseovo/last30days-skill-cn>：一个 AI Agent 技能（Skill），能够自动搜索中国互联网 8 大主流平台最近 30 天的内容，综合分析后生成有据可查的研究报告。
